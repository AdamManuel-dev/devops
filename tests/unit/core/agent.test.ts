...
describe('BaseAgent', () => {
  ...
  describe('lifecycle management', () => {
    ...
    it('should not start if already running', async () => {
      ...
      await expect(agent.start()).rejects.toThrow('Cannot start agent in state: running');
    });
...
describe('AgentRegistry', () => {
  ...
  describe('registration', () => {
    ...
    it('should throw error when registering duplicate agent', () => {
      ...
      expect(() => registry.register(agent1)).toThrow('Agent with ID agent-001 is already registered');
    });
    ...
    it('should throw error when unregistering non-existent agent', () => {
      expect(() => registry.unregister('non-existent')).toThrow('Agent with ID non-existent is not registered');
    });
  });
